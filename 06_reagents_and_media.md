# Reagents, Media, and Shared Protocols

## Version 2.0

---

## 1. Media Formulations

All formulations from Kim et al. (2025) STAR Protocols. Verify osmolality (target 950 +/- 20 mOsm/kg) and pH (7.4) for every batch. Filter all solutions through 0.20 um Nalgene Rapid-Flow units (Thermo Fisher, 0974103).

### 1.1 Molluscan Balanced Salt Solution (MBSS)

| Reagent | Concentration (w/v) | Amount (1 L) | Supplier | Catalog # |
|---|---|---|---|---|
| NaCl | 26.0 mg/mL | 26.0 g | Fisher Scientific | S671-500 |
| KCl | 1.08 mg/mL | 1.08 g | Sigma-Aldrich | P9541-1KG |
| MgCl2.6H2O | 4.70 mg/mL | 4.70 g | Sigma-Aldrich | M2670-500G |
| MgSO4.7H2O | 6.52 mg/mL | 6.52 g | Sigma-Aldrich | M2773-500G |
| NaHCO3 | 0.30 mg/mL | 300 mg | Sigma-Aldrich | S5761-1KG |
| NaH2PO4.2H2O | 0.06 mg/mL | 60 mg | MilliporeSigma | 71500-250G |
| CaCl2 (anhydrous) | 1.12 mg/mL | 1.12 g | Sigma-Aldrich | C1016 |
| D-(+)-Glucose | 0.30 mg/mL | 300 mg | MilliporeSigma | G7021-100G |

Dissolve in 1L dH2O. Store 21-25C, 6-month shelf life.

### 1.2 Molluscan Calcium/Magnesium Free Solution (MCMFS)

| Reagent | Concentration (w/v) | Amount (1 L) | Supplier | Catalog # |
|---|---|---|---|---|
| NaCl | 26.0 mg/mL | 26.0 g | Fisher Scientific | S671-500 |
| KCl | 1.08 mg/mL | 1.08 g | Sigma-Aldrich | P9541-1KG |
| NaHCO3 | 0.30 mg/mL | 300 mg | Sigma-Aldrich | S5761-1KG |
| NaH2PO4.2H2O | 0.06 mg/mL | 60 mg | MilliporeSigma | 71500-250G |
| D-(+)-Glucose | 0.30 mg/mL | 300 mg | MilliporeSigma | G7021-100G |

Dissolve in 1L dH2O. Store 21-25C, 6-month shelf life.

### 1.3 Trypsin-EDTA Solutions

| Solution | Composition | Preparation (50 mL) | Use |
|---|---|---|---|
| Solution A (dissociation) | 0.25% (w/v) trypsin (Thermo Fisher, 27250018) + 0.02% (w/v) EDTA (Sigma, 324503) in MCMFS | 125 mg trypsin + 10 mg EDTA dissolved in MCMFS to 50 mL; or use 50 mL of commercial 0.25% trypsin stock (27250018) as base + 10 mg EDTA | Tissue dissociation |
| Solution B (passaging) | 0.05% (w/v) trypsin + 0.02% (w/v) EDTA in MCMFS | 10 mL of commercial 0.25% trypsin stock (27250018) + 10 mg EDTA + 40 mL MCMFS | Lifting adherent cells |

Make fresh 50 mL aliquots. Filter 0.20 um. Store 4C, 1-month shelf life. Warm to 22C before use.

### 1.4 Squid Growth Media A (Stock)

| Reagent | Concentration (w/v) | Amount (1 L) | Supplier | Catalog # |
|---|---|---|---|---|
| NaCl | 19.67 mg/mL | 19.67 g | Fisher Scientific | S671-500 |
| KCl | 0.81 mg/mL | 810 mg | Sigma-Aldrich | P9541-1KG |
| MgCl2.6H2O | 3.53 mg/mL | 3.53 g | Sigma-Aldrich | M2670-500G |
| MgSO4.7H2O | 4.89 mg/mL | 4.89 g | Sigma-Aldrich | M2773-500G |
| NaHCO3 | 0.23 mg/mL | 230 mg | Sigma-Aldrich | S5761-1KG |
| NaH2PO4.2H2O | 0.044 mg/mL | 44 mg | MilliporeSigma | 71500-250G |
| CaCl2 (anhydrous) | 0.838 mg/mL | 838 mg | Sigma-Aldrich | C1016 |
| D-(+)-Glucose | 0.23 mg/mL | 230 mg | MilliporeSigma | G7021-100G |

Dissolve in 1L Leibovitz's L-15 medium (Thermo Fisher, 11415064). No antibiotics, no serum. Store 4C, 1-month shelf life.

### 1.5 Squid Growth Media B (Trypsin Neutralization)

| Component | Working concentration | Amount (50 mL) | Stock/Source |
|---|---|---|---|
| Penicillin/Streptomycin | 50 U/mL + 50 µg/mL | 250 µL | Gibco 15140122 (10,000 U/mL + 10,000 µg/mL stock) |
| Ciprofloxacin | 10 µg/mL | 500 µL | 1 mg/mL stock (see Section 2.1) |
| Amphotericin B | 2.5 µg/mL | 500 µL | Gibco Fungizone 15290018 (250 µg/mL stock) |
| L-15 medium | 8% (v/v) | 4.0 mL | Thermo Fisher 11415064 |
| FBS | 10% (v/v) | 5.0 mL | R&D Systems S11150H |
| Squid Growth Media A | to volume | ~39.75 mL | See Section 1.4 |

Make fresh 50 mL aliquots. Filter 0.20 um. Store 4C, 1-month shelf life.

### 1.6 Squid Growth Media C (Wash/Serum-Free)

| Component | Working concentration | Amount (50 mL) | Stock/Source |
|---|---|---|---|
| Penicillin/Streptomycin | 50 U/mL + 50 µg/mL | 250 µL | Gibco 15140122 (10,000 U/mL + 10,000 µg/mL stock) |
| Ciprofloxacin | 10 µg/mL | 500 µL | 1 mg/mL stock (see Section 2.1) |
| Amphotericin B | 2.5 µg/mL | 500 µL | Gibco Fungizone 15290018 (250 µg/mL stock) |
| L-15 medium | 8% (v/v) | 4.0 mL | Thermo Fisher 11415064 |
| Squid Growth Media A | to volume | ~44.75 mL | See Section 1.4 |

### 1.7 Squid Growth Media D (Standard Growth)

| Component | Working concentration | Amount (50 mL) | Stock/Source |
|---|---|---|---|
| Penicillin/Streptomycin | 50 U/mL + 50 µg/mL | 250 µL | Gibco 15140122 (10,000 U/mL + 10,000 µg/mL stock) |
| Ciprofloxacin | 10 µg/mL | 500 µL | 1 mg/mL stock (see Section 2.1) |
| Amphotericin B | 2.5 µg/mL | 500 µL | Gibco Fungizone 15290018 (250 µg/mL stock) |
| ITS-G | 1x (10 mg/L insulin, 5.5 mg/L transferrin, 0.0067 mg/L Na selenite) | 500 µL | Thermo Fisher 41400045 (100x liquid stock) |
| FGF-basic | 10 ng/mL | 50 µL | PeproTech 100-18B (prepare 10 µg/mL working aliquots in PBS + 0.1% BSA; store at -80C) |
| EGF | 100 ng/mL | 50 µL | PeproTech AF-100-15 (100 µg/mL stock in PBS + 0.1% BSA) |
| L-15 medium | 8% (v/v) | 4.0 mL | Thermo Fisher 11415064 |
| Squid Growth Media A | to volume | ~44.15 mL | See Section 1.4 |

Make fresh 50 mL aliquots. Filter 0.20 um. Use within 48 hours (growth factor stability).

---

## 2. Antimicrobial Strategy

### 2.1 Standard Antimicrobial Supplement (All Growth Media)

| Agent | Concentration | Target | Supplier | Catalog # |
|---|---|---|---|---|
| Penicillin | 50 U/mL | Gram-positive | Thermo Fisher (Gibco Pen-Strep) | 15140122 |
| Streptomycin | 50 ug/mL | Gram-negative (limited) | (included in above) | (included) |
| Ciprofloxacin | 10 ug/mL | Broad gram-negative (Vibrio, Pseudomonas) | Sigma-Aldrich | 17850-5G-F |
| Amphotericin B | 2.5 ug/mL | Fungi, yeast | Thermo Fisher (Gibco Fungizone) | 15290018 |

**Ciprofloxacin 100x stock**: 1 mg/mL in 0.1N HCl. Filter-sterilize. Store -20C protected from light. Add 1:100 to media.

### 2.2 Enhanced DBSS (Tissue Disinfection)

For the 30-minute tissue disinfection step. Prepare fresh as needed; 50 mL is sufficient for one dissection session.

| Component | Working concentration | Amount (50 mL) | Stock/Source |
|---|---|---|---|
| Penicillin/Streptomycin | 100 U/mL + 100 µg/mL (2x standard) | 500 µL | Gibco 15140122 (10,000 U/mL + 10,000 µg/mL stock) |
| Gentamicin | 50 µg/mL | 250 µL | Thermo Fisher 15710064 (10 mg/mL stock) |
| Ciprofloxacin | 10 µg/mL | 500 µL | 1 mg/mL stock (see Section 2.1) |
| Amphotericin B | 2.5 µg/mL | 500 µL | Gibco Fungizone 15290018 (250 µg/mL stock) |
| MBSS | base | ~48.25 mL (to 50 mL) | See Section 1.1 |

### 2.3 Pre-Dissociation Wash

Before dissection:
1. Rinse euthanized animal 3x in sterile FNSW + 100 ug/mL gentamicin.
2. Final rinse 5 min in FNSW + 100 ug/mL gentamicin + 10 ug/mL ciprofloxacin.

### 2.4 Alternative: Primocin

Primocin (InvivoGen, ant-pm-1) at 100 ug/mL can replace the multi-agent cocktail if it proves non-toxic to squid cells. Must be titrated for cytotoxicity before routine use (see Track 1 troubleshooting).

---

## 3. Euthanasia and Anesthesia

### Standard Protocol (All Experiments)

1. Prepare 7.5% MgCl2 (Fisher Scientific, M33-500) in deionized water (w/v).
2. Mix 1:1 with filtered natural seawater (FNSW). Final MgCl2: 3.75%.
3. Transfer animal to anesthetic solution.
4. Monitor 30 minutes. Confirm: cessation of chromatophore activity, loss of righting reflex, absence of respiratory movements.
5. For euthanasia: confirm death by decapitation or brain destruction.
6. For surgery (resection, transplant): proceed under anesthesia, return to clean FNSW for recovery.

This is the method used in Stock, Hally et al. for all procedures.

---

## 4. Fixation and Permeabilization

### For Cultured Cells on Glass (Standard)

1. Remove media. Wash 1x with MBSS.
2. Fix: 4% PFA in MBSS, 15 min, 22C. (Prepare fresh from 16% PFA stock, Electron Microscopy Sciences 15714-S.)
3. Wash 3x with MBSS, 5 min each.
4. Permeabilize: 0.5% Triton X-100 (Sigma, T8787) in PBS, 10 min, RT.
5. Wash 2x with PBS.
6. Block (for immunostaining): 4% BSA (MP Biomedicals, 103700) in PBS, 1 hour, RT.

### For Whole-Mount or Thick Tissue (Stock, Hally et al. Protocol)

1. Fix in 4% PFA in FNSW, 4C, 2-5 days.
2. Wash in 1% PBS-T (Tween-20).
3. Transfer to ice-cold acetone, -20C, 2 hours.
4. Wash in 1% PBS-T.
5. Permeabilize in detergent solution: 12 mM sodium deoxycholate (Sigma, D6750), 50 mM Tris pH 7.5 (Thermo, J75825.A1), 1 mM EDTA (Invitrogen, AM9261), 150 mM NaCl (Fisher, S271-3), 1% SDS (BioRad, 1610302) in DEPC-treated H2O.
6. Block: 4% BSA in PBS, 1 hour.

---

## 5. Antibody Validation Requirements

No immunofluorescence-based conclusion about cell identity may be drawn from an unvalidated antibody. The primary molecular readouts are nucleic acid-based (RT-qPCR, HCR).

### Validation Protocol (Required for Each Antibody)

1. **Bioinformatic**: align immunogen sequence to E. berryi target. >70% identity in epitope region = worth testing.
2. **Western blot**: single band at predicted MW in positive control tissue (white body), absent in negative control (mantle muscle). Multiple bands or unexpected sizes = reject.
3. **Immunofluorescence**: staining pattern on hatchling cryosections must match known mRNA expression from HCR/SOLARmap data.
4. **Controls in every experiment**: secondary-only, isotype control, and internal negative control region.

### High-Probability Cross-Reactive Antibodies

| Target | Clone/Source | Rationale |
|---|---|---|
| PCNA | Mouse anti-PCNA, clone PC10 | >85% conserved across metazoans |
| Phospho-histone H3 (Ser10) | Rabbit anti-pH3, Cell Signaling 9701 | Histone H3 essentially identical across eukaryotes |
| Acetylated alpha-tubulin | Mouse clone 6-11B-1, Sigma T7451 | alpha-tubulin extremely conserved |

---

## 6. Electrophysiology Solutions

### Extracellular Recording Solution (Marine Invertebrate)

| Reagent | Concentration | Amount (100 mL) |
|---|---|---|
| NaCl | 420 mM | 2.45 g |
| KCl | 10 mM | 74.6 mg |
| CaCl2 (anhydrous) | 10 mM | 111 mg |
| MgCl2·6H2O | 50 mM | 1.02 g |
| HEPES (free acid) | 10 mM | 238 mg |

pH 7.8 (NaOH). Osmolality ~950 mOsm/kg. Filter sterilize.

### Intracellular Recording Solution

| Reagent | Concentration | Amount (10 mL) |
|---|---|---|
| K-gluconate | 120 mM | 281 mg |
| KCl | 20 mM | 14.9 mg |
| MgCl2 | 2 mM | 20 µL of 1 M MgCl2 stock |
| EGTA | 1 mM | 3.8 mg |
| HEPES (free acid) | 10 mM | 23.8 mg |

pH 7.2 (KOH). Osmolality ~280 mOsm/kg.

The osmolality difference between extracellular (~950) and intracellular (~280) is correct and reflects the biology of marine invertebrate cells.

---

## 7. Small Molecules and Growth Factors Reference

| Compound | Working concentration | Solvent | Supplier | Catalog # | Use |
|---|---|---|---|---|---|
| CHIR99021 | 3 uM | DMSO | Tocris | 4423 | Wnt agonist (GSK3beta inhibitor) |
| Y-27632 | 10 uM | H2O | Tocris | 1254 | ROCK inhibitor (anti-anoikis) |
| Valproic acid (VPA) | 0.5 mM | H2O | Sigma | P4543 | HDAC inhibitor |
| DAPT | 10 uM | DMSO | Tocris | 2634 | Gamma-secretase inhibitor (Notch inhibition) |
| Jagged-1 Fc | 1 ug/mL | PBS | R&D Systems | 1277-JG | Notch ligand (Notch activation) |
| SAG | 1 uM | DMSO | Tocris | 4366 | Smoothened agonist (SHH pathway) |
| PD0325901 | 1 uM | DMSO | Tocris | 4192 | MEK inhibitor |
| Forskolin | 10 uM | DMSO | Tocris | 1099 | cAMP activator |
| FGF-basic (154 aa) | 10-50 ng/mL | PBS + 0.1% BSA | PeproTech | 100-18B | NSC mitogen |
| EGF (human, animal-free) | 100 ng/mL | PBS + 0.1% BSA | PeproTech | AF-100-15 | NSC mitogen |
| SCF (human) | 50 ng/mL | PBS + 0.1% BSA | PeproTech | 300-07 | Hematopoietic growth factor |
| TPO (human) | 50 ng/mL | PBS + 0.1% BSA | PeproTech | 300-18 | Hematopoietic growth factor |
| BDNF (human) | 20 ng/mL | PBS + 0.1% BSA | PeproTech | 450-02 | Neurotrophin (maturation) |
| NT-3 (human) | 20 ng/mL | PBS + 0.1% BSA | PeproTech | 450-03 | Neurotrophin (maturation) |
| ABT-263 (navitoclax) | 1 uM | DMSO | Selleckchem | S1001 | Senolytic |
| Rapamycin | 10 nM | DMSO | Sigma | R0395 | mTOR inhibitor |
| Cycloastragenol (TA-65) | 10 uM | DMSO | Sigma | SML0986 | Telomerase activator |

---

## 8. Key Consumables

| Item | Supplier | Catalog # |
|---|---|---|
| 24-well glass bottom plates | Cellvis | P24-1.5H-N |
| 96-well glass bottom plates | Cellvis | P96-1.5H-N |
| Nalgene 0.20 um filter units (1L) | Thermo Fisher | 0974103 |
| Minutens insect pins (12 x 0.20 mm) | Indigo Instruments | 33416-20 |
| Insect pins #3 (38 x 0.50 mm) | Indigo Instruments | 33414-3 |
| SYLGARD 184 silicone (dissection dish) | Krayden | DC4019862 |
| Dumostar-Biology #55 forceps | Fine Science Tools | 11295-51 |
| Vanas Spring Scissors | Fine Science Tools | 15000-08 |
| Cell scrapers (25cm handle, 20mm blade) | CELLTREAT | 229310 |
| Lipofectamine MessengerMAX | Thermo Fisher | LMRNA003 |
| Lipofectamine 3000 | Thermo Fisher | L3000001 |
| Opti-MEM | Thermo Fisher | 31985062 |
| F-ara-EdU | Vector Laboratories | CCT-1403-5 |
| AF647 fluorescent azide | Vector Laboratories | CCT-1284-1 |
| Accutase | StemCell Technologies | 07920 |
